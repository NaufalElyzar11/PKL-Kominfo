...
<div class="flex flex-col gap-6" x-data="{ 
    ...
    rejectId: null, 
    isPejabatConfirmedForReject: false 
}">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {{-- =========================================================== --}}
        {{-- BAGIAN 1: DAFTAR PENGAJUAN MENUNGGU (LOGIKA UTAMA DISINI) --}}
        {{-- =========================================================== --}}
        <div>
                    ...
                    <table class="w-full text-xs text-left text-gray-800">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="bg-white hover:bg-blue-50/30 transition-colors group" 
                                ...
                                {{-- TD KONFIRMASI PEJABAT --}}
                                <td class="px-4 py-3 text-center border-x border-gray-50">
                                    <label class="inline-flex flex-col items-center gap-1 cursor-pointer group/check">
                                        <input type="checkbox" x-model="isConfirmed" 
                                            class="w-5 h-5 text-emerald-600 rounded border-gray-300 focus:ring-emerald-500 transition-all shadow-sm">
                                        <span class="text-[9px] font-extrabold uppercase tracking-tighter" 
                                            :class="isConfirmed ? 'text-emerald-600' : 'text-slate-300'"
                                            x-text="isConfirmed ? 'Sudah Izin' : 'Belum Izin'"></span>
                                    </label>
                                </td>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form id="form-approve-{{ $c->id }}" action="{{ route('admin.cuti.approve', $c->id) }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="pejabat_confirmed" :value="isConfirmed ? 1 : 0">
                                            <button type="button" 
                                                    @click="if(isConfirmed) confirmApprove('{{ $c->id }}')" 
                                                    :disabled="!isConfirmed"
                                                    :class="!isConfirmed ? 'opacity-20 cursor-not-allowed bg-gray-200 text-gray-400' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-500 hover:text-white'"
                                                    class="w-7 h-7 rounded-full transition-all flex items-center justify-center shadow-sm">
                                        ...
                                        {{-- Tombol Reject (X) --}}
                                        <button type="button" 
                                                @click="rejectId = '{{ $c->id }}'; isPejabatConfirmedForReject = isConfirmed; showRejectModal = true" 
                                                :disabled="!isConfirmed"
                                                :class="!isConfirmed ? 'opacity-20 cursor-not-allowed bg-gray-200 text-gray-400' : 'bg-rose-50 text-rose-600 hover:bg-rose-500 hover:text-white shadow-rose-100'"
                                                class="w-7 h-7 rounded-full transition-all flex items-center justify-center shadow-sm" 
                                                title="Tolak">
                            ...
                            @empty
                            <tr><td colspan="9" class="px-4 py-8 text-center text-gray-400 italic">Tidak ada pengajuan cuti yang menunggu.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
        ...
        {{-- 2. MODAL ALASAN PENOLAKAN (WAJIB ADA DI DALAM DIV X-DATA UTAMA) --}}
        <div x-show="showRejectModal" x-transition.opacity class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/50" x-cloak>
            <div class="bg-white rounded-lg shadow-xl w-full max-w-md p-6" @click.away="showRejectModal = false">
                <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                    <i class="fa-solid fa-triangle-exclamation text-rose-500"></i> Alasan Penolakan
                </h3>
                <form :action="'{{ route('admin.cuti.reject', ['id' => '_id']) }}'.replace('_id', rejectId)" method="POST">
                    @csrf
                    <input type="hidden" name="pejabat_confirmed" :value="isPejabatConfirmedForReject ? 1 : 0">
                    <div x-data="{ count: 0 }">
                        <textarea 
                            ...
                            required 
                            maxlength="100" 
                            {{-- Logika: Hapus semua karakter yang BUKAN huruf (a-z) atau spasi (\s) --}}
                            @input="$event.target.value = $event.target.value.replace(/[^a-zA-Z\s]/g, ''); count = $event.target.value.length" 
                            class="w-full border-gray-300 rounded-md p-2 border text-sm" 
                            placeholder="Tunda cuti dulu karena sedang banyak kerjaan..."></textarea>
                        <div class="text-xs text-gray-500 text-right mt-1"><span x-text="count"></span>/100</div>
                    </div>
                    <div class="mt-6 flex justify-end space-x-3">
                        <button type="button" @click="showRejectModal = false" class="text-gray-600 px-4 py-2 text-sm">Batal</button>
                        <button type="submit" class="bg-rose-600 text-white px-4 py-2 rounded-md text-sm font-semibold hover:bg-rose-700">Kirim Penolakan</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        {{-- =========================================================== --}}
        {{-- BAGIAN 2: RIWAYAT PENGAJUAN (HANYA UNTUK TAMPILAN) --}}
        {{-- =========================================================== --}}
        <div>
            <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                    ...
                    <table class="w-full text-xs text-left text-gray-800">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="bg-white hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-3">{{ optional($r->tanggal_mulai)->format('d M Y') }} s.d. {{ optional($r->tanggal_selesai)->format('d M Y') }}</td>
                                <td class="px-4 py-3 text-center">
                                    <span class="px-2 py-0.5 rounded-full text-[10px] font-bold {{ $r->status == 'Disetujui' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        {{ $r->status }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-gray-500 italic">{{ $r->catatan_final ?? '-' }}</td>
                            </tr>
                            @empty
                            <tr><td colspan="6" class="px-4 py-6 text-center text-gray-500 italic">Belum ada riwayat pengajuan.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
